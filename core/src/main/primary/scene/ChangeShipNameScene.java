...
public class ChangeShipNameScene extends Scene {

    public void create() {
        table.defaults().pad(70f);

        // Title
        table.add(title("Ship Name (12 characters max)", Color.YELLOW));
        ...
        table.add(textButton("Continue", Color.RED, () -> {
            if (nameInput.getText() == null || nameInput.getText().trim().isEmpty()) {
                ...
                Global.app.player.getShip().setName(nameInput.getText());
                sceneLoader.setScene(new ShipScene());
            }
        })).pad(10);
        table.row();

        // Go back to Ship scene
